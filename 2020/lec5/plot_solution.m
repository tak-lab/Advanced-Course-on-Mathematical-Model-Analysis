...
b_pad = [zeros(m_pad,1);b;zeros(m_pad,1)];
N_pad = N+m_pad;
% k = (-N_pad:N_pad)';
dx = L/(2*N_pad-1);
x = dx*(0:2*N_pad);
...
if index==1
  ...
  subplot(1,2,2)
  plot(x,real((2*N_pad+1)*ifft(ifftshift(b_pad))),'linewidth',1.6)
...
elseif index==3
  plot(real((2*N_pad+1)*ifft(ifftshift(a_pad))),real((2*N_pad+1)*ifft(ifftshift(b_pad))),'linewidth',1.6)
